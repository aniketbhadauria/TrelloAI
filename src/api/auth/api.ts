...
export async function apiSignIn(email: string, password: string): Promise<AuthResult> {
  ...
  if (error) {
    logError('sign_in_failed', { message: error.message })
    throw error
  }
...
export async function apiSignUp(email: string, password: string): Promise<AuthResult> {
  ...
  if (error) {
    logError('sign_up_failed', { message: error.message })
    throw error
  }
...
export async function apiSignOut(): Promise<void> {
  ...
  if (error) {
    logError('sign_out_failed', { message: error.message })
    throw error
  }
  logInfo('sign_out_success')
}

export async function apiSyncAppUser(
  userId: string,
  email: string,
  displayName: string
): Promise<void> {
