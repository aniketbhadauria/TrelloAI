...
export async function apiFetchAllBoards(userId: string): Promise<FetchBoardsResult> {
  ...
  if (ownedResult.error) logError('boards_fetch_owned_failed', { message: ownedResult.error.message, userId });
  if (sharedResult.error) logError('boards_fetch_shared_failed', { message: sharedResult.error.message, userId });
  ...
  const sharedRows = (
    (sharedResult.data || []).filter((m: Record<string, unknown>) => m.boards) as unknown
  ) as Array<{
    board_id: string;
    role: BoardRole;
