...
const ROLE_META: Record<BoardRole, RoleMeta> = {
...
export default function BoardMembersPanel({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cancelled) return;
      setMembers((memberships || []).map((m: { user_id: string; role: BoardRole; invited_at: string }) => ({
        ...m,
        profile: profiles.find((p: MemberProfile) => p.id === m.user_id) || null,
      })));
      setLoading(false);
    })();
  ...
  const handleRoleChange = async (userId: string, newRole: BoardRole): Promise<void> => {
  ...
  const existingMemberIds = [ownerId, ...members.map(m => m.user_id)].filter(Boolean) as string[];
          ...
          {loading ? (
            ...
            <div className="space-y-1 mb-4">
              ...
              <div className="flex items-center gap-3 px-1 py-2 rounded-lg">
                <Avatar name={ownerDisplayName as string} />
                ...
                return (
                  <div key={m.user_id} className="flex items-center gap-3 px-1 py-2 rounded-lg hover:bg-secondary/30 group">
                    ...
                    {canManage ? (
                      <select
                        value={m.role}
                        onChange={(e: React.ChangeEvent<HTMLSelectElement>) => handleRoleChange(m.user_id, e.target.value as BoardRole)}
