...
const MacOSDock = ({ apps, onAppClick, openApps = [], className = '' }) => {
  ...
  const animateToTarget = useCallback(() => {
    ...
    setCurrentScales(prevScales => prevScales.map((s, i) => s + (targetScales[i] - s) * lerpFactor));
    setCurrentPositions(prevPositions => prevPositions.map((p, i) => p + (targetPositions[i] - p) * lerpFactor));
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: `0 ${Math.max(4, baseIconSize * 0.1)}px ${Math.max(16, baseIconSize * 0.4)}px rgba(0,0,0,0.4), 0 ${Math.max(2, baseIconSize * 0.05)}px ${Math.max(8, baseIconSize * 0.2)}px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.15), inset 0 -1px 0 rgba(0,0,0,0.2)`,
        padding: `${padding}px`,
      }}
