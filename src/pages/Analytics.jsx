...
import {
  ...
  ChevronDown, Activity, Flame, Clock, ArrowUpRight,
  Crown, Shield, Target, Award, PieChart as PieChartIcon,
  Zap, UserCheck, Layers,
} from 'lucide-react';
...
export default function Analytics() {
  ...
  const analytics = useMemo(() => {
    ...
    for (const board of filteredBoards) {
      for (const list of board.lists || []) {
        ...
        for (const card of list.cards || []) {
          ...
          if (members.length === 0) {
            if (!memberTasks['Unassigned']) memberTasks['Unassigned'] = { total: 0, done: 0, inProgress: 0, todo: 0, review: 0, other: 0, overdue: 0, onTime: 0 };
          ...
          for (const member of members) {
            ...
            if (!memberTasks[name]) memberTasks[name] = { total: 0, done: 0, inProgress: 0, todo: 0, review: 0, other: 0, overdue: 0, onTime: 0 };
    ...
    const statusData = Object.entries(statusCounts)
      .filter(([, v]) => v > 0)
      .map(([key, value]) => ({
        name: key === 'todo' ? 'To Do' : key === 'inProgress' ? 'In Progress' : key === 'done' ? 'Done' : key === 'review' ? 'Review' : 'Other',
        value,
    ...
    <div className="analytics-page p-5 md:p-8 max-w-[1440px] mx-auto page-enter pb-16">
      ...
      {!hasData ? (
        <div className="glass-card flex flex-col items-center justify-center py-24 text-center">
          <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-pink-100 to-purple-100 flex items-center justify-center mb-5">
                ...
                <div className="flex flex-col lg:flex-row items-center gap-4">
                  <div className="flex-1 w-full">
                    <ResponsiveContainer width="100%" height={280}>
                      <PieChart>
                        <Pie data={analytics.workloadData} cx="50%" cy="50%" outerRadius={105} innerRadius={60} paddingAngle={3} dataKey="value" animationBegin={0} animationDuration={900} strokeWidth={0}>
                          {analytics.workloadData.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
                        </Pie>
                        <Tooltip content={<GlassTooltip />} />
                      </PieChart>
                    </ResponsiveContainer>
                  </div>
                  {/* Legend as glass pills */}
                  <div className="flex flex-col gap-2 min-w-[160px]">
                    {analytics.workloadData.map((d, i) => (
                      <div key={d.name} className="glass-inner flex items-center gap-2.5 !p-2.5 !rounded-xl">
              ...
              {analytics.uniqueMembers.length > 0 && analytics.monthlyBarData.some(r => Object.values(r).some(v => typeof v === 'number' && v > 0)) ? (
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={analytics.monthlyBarData} barGap={2} barCategoryGap="18%">
                    ...
                    <Tooltip content={<GlassTooltip />} />
                    <Legend iconType="circle" iconSize={8} formatter={(value) => <span style={{ fontSize: 11, color: '#6b7280', fontWeight: 500 }}>{value}</span>} />
...
function MemberCard({ member, index }) {
  const cfg = {
    overloaded: { label: 'Overloaded', pillCls: 'pill-red', dotColor: '#ef4444', barColor: '#ef4444', icon: <Flame className="w-3 h-3" /> },
    busy:       { label: 'Busy',       pillCls: 'pill-amber', dotColor: '#f59e0b', barColor: '#f59e0b', icon: <Clock className="w-3 h-3" /> },
    available:  { label: 'Available',  pillCls: 'pill-green', dotColor: '#10b981', barColor: '#10b981', icon: <Shield className="w-3 h-3" /> },
  }[member.status];
  ...
  return (
    <div className="glass-inner flex items-center gap-3 !p-3 !rounded-xl hover:scale-[1.01] transition-transform duration-200 cursor-default">
      ...
      <div className="text-right shrink-0">
        <p className="text-sm font-bold bg-gradient-to-r from-pink-500 to-purple-500 bg-clip-text text-transparent">{member.rate}%</p>
      </div>
    </div>
  );
}
